...
class TreeDiagram(AnimatableComponent):
    def __init__(self, root_label=None, children_labels=None, **kwargs):
        ...
        for child in self.children_nodes:
            self.edges.add(Line(self.root.get_bottom(), child.get_top(), color=VisualGrammar.colors["accent"]))
        self.add(self.root, self.children_nodes, self.edges)

    def get_intro_animations(self):
        anims = [FadeIn(self.root, shift=DOWN*0.2)]
        for child, edge in zip(self.children_nodes, self.edges):
            anims.append(AnimationGroup(Create(edge), FadeIn(child, shift=UP*0.2), lag_ratio=0.5))
        return [LaggedStart(*anims, lag_ratio=0.5)]
        
    def get_highlight_animations(self, target=None):
        if target:
            target_str = str(target).lower()
            if "root" in target_str:
                return [Indicate(self.root, color=VisualGrammar.colors["accent"])]
            for child in self.children_nodes:
                if target_str in child[1].t.original_text.lower():
                    return [Indicate(child, color=VisualGrammar.colors["accent"])]
        return [Indicate(self.root, color=VisualGrammar.colors["accent"])]
        
    def get_transformation_animations(self, target=None):
        return [Wiggle(child) for child in self.children_nodes]
        
    def get_explanation_animations(self, target=None):
        return [Indicate(self.edges, color=VisualGrammar.colors["accent"])]
        
    def get_focus_animations(self, target=None):
        ...
        for i in range(len(self.all_nodes)-1):
            for src in self.all_nodes[i]:
                for tgt in self.all_nodes[i+1]:
                    self.edges.add(Line(src.get_center(), tgt.get_center(), stroke_opacity=0.4, color=VisualGrammar.colors["muted"], z_index=-1))
